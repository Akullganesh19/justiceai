 ...
 * Retries on network errors, 5xx, or 429 status codes.
 * Returns the final failed Response object if all retries are exhausted.
 */
async function fetchWithRetry(url, options = {}, maxRetries = 3) {
  const { timeout, ...fetchOptions } = options;
  const isIdempotent = !fetchOptions.method || ['GET', 'HEAD', 'OPTIONS', 'PUT', 'DELETE'].includes(fetchOptions.method.toUpperCase());
  const isStatelessPost = fetchOptions.method && fetchOptions.method.toUpperCase() === 'POST';
  const shouldRetry = isIdempotent || isStatelessPost;
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    ...
    try {
      ...
      // Retry on transient errors (5xx, 429) if applicable
      if (!response.ok && shouldRetry && (response.status >= 500 || response.status === 429)) {
        if (attempt === maxRetries) {
          return response; // Return final failed response
        }

        const delay = 1000 * Math.pow(2, attempt - 1);
        console.warn(`[fetchWithRetry] Attempt ${attempt} failed for ${url} with status ${response.status}. Retrying in ${delay}ms...`);
      ...
      // Retry on network errors or timeouts (AbortError)
      if (shouldRetry) {
        if (attempt === maxRetries) throw err;

        const delay = 1000 * Math.pow(2, attempt - 1);
        const errName = err.name || 'Error';
        console.warn(`[fetchWithRetry] Attempt ${attempt} failed for ${url} with ${errName}. Retrying in ${delay}ms...`);
        await new Promise(res => setTimeout(res, delay));
      } else {
        throw err;
      }
