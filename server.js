  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      if (attempt === maxAttempts) {
        console.error(`Max retries reached for ${url} (HTTP ${response.status})`);
        return response; // Return final failed response, don't throw
      }

      console.warn(`Attempt ${attempt} failed for ${url} (HTTP ${response.status}). Retrying...`);
      ...
      if (!isRetryable || attempt === maxAttempts) {
        if (attempt === maxAttempts) {
          console.error(`Max retries reached for ${url}: ${err.message}`);
        }
        throw err;
      }
