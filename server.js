...
// 🧬 Genesis: Auto-Retry with Exponential Backoff for resilient external calls
async function fetchWithRetry(url, options = {}, maxAttempts = 3) {
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      // Log retry and backoff on 429 or 5xx
      logger.warn(`Attempt ${attempt} failed with status ${response.status}. Retrying...`);
    } catch (err) {
      if (err.name === 'AbortError') throw err; // Don't catch intentional timeouts/aborts
      if (attempt === maxAttempts) throw err;
      logger.warn(`Attempt ${attempt} failed with network error: ${err.message}. Retrying...`);
    }
