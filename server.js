...
// ==========================================
// 🧬 GENESIS SELF-HEALING: Auto-Retry for External APIs
// ==========================================
...
async function fetchWithRetry(url, options = {}, maxAttempts = 3) {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      // If response is successful, or it's not a transient error, return it immediately
      if (response.ok || (response.status !== 429 && response.status < 500)) {
        return response;
      }

      // Transient HTTP errors (429, 5xx)
      if (attempt === maxAttempts) return response; // Last attempt, return the error response

      console.warn(`[Genesis] Transient error ${response.status} from ${safeUrl}. Retrying (${attempt}/${maxAttempts})...`);

      // Explicitly consume the body of the failed response to prevent memory leaks
      ...
      // Don't retry AbortError (e.g. timeout)
      ...
      const isNetworkError = code === 'ECONNREFUSED' || code === 'ECONNRESET' || code === 'ENOTFOUND' || code === 'ETIMEDOUT' || code === 'UND_ERR_CONNECT_TIMEOUT';

      if (attempt === maxAttempts || !isNetworkError) {
        throw err;
      }

      console.warn(`[Genesis] Network error (${code}) fetching ${safeUrl}. Retrying (${attempt}/${maxAttempts})...`);
      await sleep(100 * Math.pow(2, attempt - 1));
    }
  }
}
