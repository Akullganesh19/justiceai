...
// Self-healing wrapper for transient network/API failures
async function fetchWithRetry(url, options = {}, maxRetries = 3) {
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      const response = await fetch(url, options);
      // If OK, or if it's the last attempt, return
      if (response.ok || attempt === maxRetries) {
        return response;
      }
      // Only retry rate limits (429) or server errors (5xx)
      if (response.status !== 429 && response.status < 500) {
        return response;
      }
      const delay = 100 * Math.pow(2, attempt - 1);
      logger.warn(`API call failed (Status: ${response.status}). Retrying... (${attempt}/${maxRetries}) in ${delay}ms`);
      ...
      const delay = 100 * Math.pow(2, attempt - 1);
      logger.warn(`API call network error (${err.message}). Retrying... (${attempt}/${maxRetries}) in ${delay}ms`);
      await new Promise(res => setTimeout(res, delay));
    }
  }
}
