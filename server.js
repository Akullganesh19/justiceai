...
// 🧬 Genesis: Auto-retry wrapper for resilient external API calls
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    ...
    try {
      ...
      if (!response.ok && (response.status >= 500 || response.status === 429) && attempt < maxRetries) {
        // Explicitly consume body before retry to prevent memory leak
      ...
      if (err.name === 'AbortError' || err.message.includes('Transient HTTP') || err.cause || err.code === 'ECONNRESET' || err.code === 'ENOTFOUND') {
        ...
        console.warn(`[Genesis] Transient failure fetching ${safeUrl} (${err.message}). Retrying (${attempt}/${maxRetries}) in ${backoff}ms...`);
