...
// Self-Healing Mechanism: Auto-Retry with Exponential Backoff
async function fetchWithRetry(url, options = {}, maxAttempts = 3) {
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      const res = await fetch(url, options);
      if (!res.ok && res.status >= 500 && attempt < maxAttempts) {
        logger.warn(`${url} returned ${res.status}, retrying (${attempt}/${maxAttempts})...`);
      ...
      if (err.name === 'AbortError') {
        throw err; // Never retry explicit cancellations
      }
      ...
      if (isNetworkError && attempt < maxAttempts) {
        logger.warn(`Network error calling ${url}, retrying (${attempt}/${maxAttempts}): ${err.message}`);
        await new Promise(resolve => setTimeout(resolve, 100 * Math.pow(2, attempt - 1)));
        continue;
      }

      // If it's the last attempt or an error we don't want to retry, throw immediately
      throw err;
    }
  }
}
