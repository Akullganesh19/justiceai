...
async function fetchWithRetry(url, options = {}, maxAttempts = 3) {
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      // If it is a 5xx error, it is transient, we can retry
      if (response.status >= 500 && response.status < 600) {
        const errText = await response.text();
        throw new Error(`HTTP ${response.status} - ${errText}`);
      }

      // 4xx errors are generally not transient, do not retry
      return response;
    } catch (err) {
      if (attempt === maxAttempts) throw err;
      if (err.name === "AbortError") throw err;

      const isTransient = err.message.includes("fetch failed") || err.message.includes("HTTP 5");
      if (!isTransient && err.name !== "TypeError") throw err;

      const delay = 100 * Math.pow(2, attempt - 1);
      const safeUrl = url.split("?")[0];
      console.warn(`[Retry] fetch ${safeUrl} failed (attempt ${attempt}/${maxAttempts}): ${err.message}. Retrying in ${delay}ms...`);
