  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      if (attempt === maxAttempts) {
        logger.error(`Failed after ${maxAttempts} attempts: ${url}`, { error: err.message });
        throw err;
      }
      const backoff = 100 * Math.pow(2, attempt - 1);
      logger.warn(`Attempt ${attempt} failed for ${url}. Retrying in ${backoff}ms...`, { error: err.message });
