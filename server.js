...
const redactFormatter = winston.format((info) => {
  ...
  const ccRegex = /\b(?:4\d{15}|5[1-5]\d{14}|3[47]\d{13}|6011\d{12}|65\d{14})\b/g;
  const phoneRegex = /(?:\+?(\d{1,3}))?[-. (]*(\d{3})[-. )]*(\d{3})[-. ]*(\d{4})(?: *x(\d+))?\b/g;
  ...
  const traverseAndRedact = (obj, seen) => {
    ...
    for (const key of Object.keys(obj)) {
      redactedObj[key] = traverseAndRedact(obj[key], seen);
    }
    return redactedObj;
  };

  const seen = new WeakSet();
  const redactedInfo = traverseAndRedact(info, seen);
  ...
  if (typeof redactedInfo.message === 'string') {
    redactedInfo.message = mask(redactedInfo.message);
  }
...
app.post('/api/chat', async (req, res) => {
  ...
  try {
    ...
    logger.info(`Routing query to Ollama`);
