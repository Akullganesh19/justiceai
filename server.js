  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      if (!response.ok && (response.status === 429 || response.status >= 500)) {
        ...
        logger.warn(`[Genesis] ⚠️ Transient error ${response.status} from ${url}. Retrying (${attempt}/${maxAttempts}) in ${delay}ms...`);
      ...
      const delay = 100 * Math.pow(2, attempt - 1);
      logger.warn(`[Genesis] ⚠️ Network error (${err.message}) calling ${url}. Retrying (${attempt}/${maxAttempts}) in ${delay}ms...`);
      await new Promise(r => setTimeout(r, delay));
    }
  }
}
