...
async function fetchWithRetry(url, options = {}, maxAttempts = 3) {
  const { timeout, retryNonIdempotent, ...fetchOptions } = options;
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      const fetchCallUrl = isRequest ? url.clone() : url;
      const attemptOptions = { ...fetchOptions };

      if (timeout) attemptOptions.signal = AbortSignal.timeout(timeout);

      const response = await fetch(fetchCallUrl, attemptOptions);
      ...
      logger.warn(`[Retry] Attempt ${attempt} failed for ${sanitizedUrl}: ${err.message}. Retrying...`);
