...
// 🧬 Genesis: Self-healing fetch wrapper with exponential backoff for transient errors
async function fetchWithRetry(url, options = {}, maxAttempts = 3, baseDelayMs = 100) {
  ...
  while (true) {
    ...
    try {
      ...
      logger.warn(`Attempt ${attempt} failed for ${url}, retrying in ${delay}ms...`, { error: err.message });
    ...
    if (!response.ok && [429, 500, 502, 503, 504].includes(response.status) && attempt < maxAttempts) {
      // Consume the body of the failed response to prevent memory leaks in Node.js
      await response.text().catch(() => {});

      const delay = baseDelayMs * Math.pow(2, attempt - 1);
      logger.warn(`Attempt ${attempt} got ${response.status} for ${url}, retrying in ${delay}ms...`);
      await new Promise(res => setTimeout(res, delay));
      attempt++;
      continue;
    }
