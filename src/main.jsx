...
// Mapping of route paths to their dynamic import functions
// This serves both React.lazy and our predictive prefetch engine
const routeChunks = {
...
// We detect intent before action: when a user hovers or moves toward a link,
// we immediately begin downloading the JavaScript chunk for that route.
// By the time they click (~200-400ms later), the page logic is already in memory.
if (typeof window !== 'undefined') {
  const prefetchRoute = (e) => {
    ...
    try {
      ...
      // Only prefetch our own origins
      if (url.origin !== window.location.origin) return;

      const path = url.pathname;
      // Find matching route or check if it matches a parameterized route (not common here, but safe)
      const importFn = routeChunks[path] || Object.entries(routeChunks).find(([key]) => key !== '/' && key !== '*' && path.startsWith(key + '/'))?.[1];

      if (importFn) {
        // Trigger the webpack/vite chunk download in the background.
        // The .catch swallows errors so network blips don't crash the user session.
        importFn().catch(() => {});
      }
    } catch (_err) {
  ...
  // Passive listeners ensure we never block scrolling or rendering performance
