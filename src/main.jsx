...
// PREFETCH_MAP enables predictive fetching of JS chunks before users click
const PREFETCH_MAP = {
...
// Lazy-loaded pages utilizing PREFETCH_MAP
...
// Oracle Predictive Intelligence Component
function OraclePrefetcher() {
  const location = useLocation();

  useEffect(() => {
    // Prediction graph: When user is at 'current_path', they are likely to visit 'predicted_paths' next
    const TRANSITIONS = {
      '/': ['/dashboard', '/chat'],
      '/dashboard': ['/chat', '/tracker', '/documents'],
      '/chat': ['/documents', '/tracker'],
      '/documents': ['/chat', '/tracker'],
      '/tracker': ['/documents', '/chat'],
      '/legal-aid': ['/lawyers'],
    };

    const nextPaths = TRANSITIONS[location.pathname];
    if (nextPaths) {
      nextPaths.forEach(path => {
        if (PREFETCH_MAP[path]) {
          // Execute the dynamic import in background to warm up Vite/React lazy cache
  ...
  return null; // Silent infrastructure component
}
