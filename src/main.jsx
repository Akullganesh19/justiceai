...
// Mapping of route paths to their dynamic import chunks.
// Used for both React.lazy registration and predictive prefetching.
const routeChunks = {
...
// 🛸 ORACLE: Predictive Intelligent Prefetching
// Listen for hover events to anticipate route transitions.
    ...
    if (anchor && anchor.href) {
      try {
        ...
        if (url.origin === window.location.origin) {
          // Normalize path by removing query params and hash fragments
          const path = url.pathname;

          if (routeChunks[path]) {
            // Predictively load the chunk in the background so it's ready if clicked
            routeChunks[path]().catch(() => {
               // Swallow loading errors to not disrupt user experience
            });
          }
        }
      } catch (_err) {
