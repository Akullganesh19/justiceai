...
// Centralized mapping of routes to dynamic imports for component lazy loading and predictive prefetching.
const routeChunks = {
...
// Global event listener to prefetch lazily loaded React route chunks on hover or touch.
// This improves perceived performance by predicting where the user is going.
    ...
    if (link && link.href) {
      try {
        ...
        if (url.origin === window.location.origin) {
          ...
          let importFn = routeChunks[path];
          if (!importFn) {
            // Find prefix matches for parameterized routes
            for (const key in routeChunks) {
              if (key !== '/' && key !== '*' && path.startsWith(key + '/')) {
                importFn = routeChunks[key];
                break;
              }
            }
          }
          ...
          if (importFn) {
            importFn().catch((_err) => {
              // Swallow load errors to prevent disrupting user experience
            });
          }
        }
      } catch (_err) {
