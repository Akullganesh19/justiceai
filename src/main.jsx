...
// Map of dynamic imports for predictive route prefetching
const routeChunks = {
...
// Oracle Predictive Prefetching: Detect intent to navigate and preload chunks
    ...
    if (link && link.href) {
      try {
        ...
        if (url.origin === window.location.origin) {
          // Normalize path by stripping query params and hashes
          const normalizedPath = url.pathname.split('?')[0].split('#')[0];
          const chunkLoader = routeChunks[normalizedPath];
          if (chunkLoader) {
            // Eagerly execute the import, swallow errors to degrade gracefully
            chunkLoader().catch(() => {});
          }
        }
      } catch (_err) {
