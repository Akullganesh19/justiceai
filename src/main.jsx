...
// Oracle Predictive Route Prefetching Map
// We store the dynamic imports centrally so we can invoke them manually before React renders them.
const routeChunks = {
...
// 🛸 Oracle: Predictive Prefetch Engine
// Anticipates user navigation based on hover/touch intents and pre-loads the route chunk.
if (typeof window !== 'undefined') {
  const prefetchRoute = (urlStr) => {
    try {
      ...
      for (const [key, importFn] of Object.entries(routeChunks)) {
        if (pathname === key || (key !== '/' && pathname.startsWith(key + '/'))) {
          // Fire and forget, catching errors to degrade gracefully if chunk fails to load
          importFn().catch(() => {});
          break;
        }
      }
  ...
  const handlePredictiveIntent = (e) => {
  ...
  window.addEventListener('mouseover', handlePredictiveIntent, { passive: true });
  window.addEventListener('touchstart', handlePredictiveIntent, { passive: true });
}

// Lazy-loaded pages for optimal bundle splitting, using the prefetchable map
