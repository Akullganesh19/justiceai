...
// Centralized map of route paths to their dynamic import functions
// Used for both React.lazy registration and Oracle predictive prefetching
const routeChunks = {
...
// 🛸 ORACLE PREDICTION ENGINE: Intelligent Route Prefetching
// Predicts what chunk the user needs *before* they click
if (typeof window !== 'undefined') {
  const handlePrefetch = (e) => {
    ...
    try {
      ...
      if (url.origin === window.location.origin) {
        ...
        // Find exact match or prefix match (for parameterized routes)
        for (const [key, importFn] of Object.entries(routeChunks)) {
          if (path === key || (key !== '/' && key !== '*' && path.startsWith(key + '/'))) {
            matchedChunk = importFn;
            break;
          }
        }

        // Execute background fetch if chunk found
        if (matchedChunk) {
          // Swallow errors to degrade gracefully
          matchedChunk().catch(() => {});
        }
      }
    } catch (_err) {
  ...
  // Listen to both hover and mobile touch intent
  window.addEventListener('mouseover', handlePrefetch, { passive: true });
  window.addEventListener('touchstart', handlePrefetch, { passive: true });
}

// Lazy-loaded pages for optimal bundle splitting using the map
