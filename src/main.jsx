...
// Intermediate component to handle additive migration (Context + Event)
function GlobalVoiceBridge() {
  const { setGlobalTranscription } = useTranscription();

  const handleGlobalTranscription = (text) => {
    // 1. Update the new React Context state
    setGlobalTranscription(text);

    // 2. Dispatch legacy CustomEvent for backward compatibility during migration
