...
// Centralized mapping of paths to route chunks for predictive prefetching
const routeChunks = {
...
// Intelligent route prefetching: Load chunks when users express intent to navigate
if (typeof window !== 'undefined') {
  const prefetchChunk = (e) => {
    ...
    try {
      ...
      let chunkToLoad = routeChunks[path];

      if (!chunkToLoad) {
        // Fallback to directory prefix matching if exact match not found
        const match = Object.keys(routeChunks).find(key => key !== '/' && key !== '*' && path.startsWith(key + '/'));
        if (match) chunkToLoad = routeChunks[match];
      }

      if (chunkToLoad) {
        chunkToLoad().catch(() => {}); // Swallow potential loading errors gracefully
      }
    } catch (_err) {
  ...
  window.addEventListener('mouseover', prefetchChunk, { passive: true });
  window.addEventListener('touchstart', prefetchChunk, { passive: true });
}
