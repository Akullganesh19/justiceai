...
// Centralized map for route chunk prefetching
const routeChunks = {
...
// 🛸 Oracle: Predictive hover-to-prefetch system
// When user hovers toward a link, we start fetching the chunk before they click
    ...
    if (link && link.href) {
      try {
        ...
        if (url.origin === window.location.origin) {
          ...
          // Normalize path: strip query params and hash (though pathname already strips query/hash)
          // Just to be absolutely safe:
          const normalizedPath = path.split('?')[0].split('#')[0];

          if (routeChunks[normalizedPath]) {
            // Initiate background load, ignore errors (graceful degradation)
