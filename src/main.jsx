...
if (typeof window !== 'undefined') {
  const prefetchRoute = (e) => {
    ...
    try {
      ...
      if (!matchedImport) {
        // Try directory prefix matching for parameterized routes
        const matchedKey = Object.keys(routeChunks).find(
          key => key !== '/' && key !== '*' && path.startsWith(key + '/')
        );
        if (matchedKey) matchedImport = routeChunks[matchedKey];
      }

      if (matchedImport) {
        matchedImport().catch(() => {}); // Swallow chunk loading errors silently
      }
    } catch (_err) {
