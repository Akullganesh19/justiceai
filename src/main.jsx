...
// Oracle Predictive Prefetching Configuration
const routeChunks = {
...
if (typeof window !== 'undefined') {
  const prefetchRoute = (e) => {
    ...
    try {
      ...
      if (url.origin === window.location.origin) {
        ...
        // Prefix match for nested routes if exact match not found
        if (!importFn) {
           for (const [key, fn] of Object.entries(routeChunks)) {
        ...
        if (importFn) {
          importFn().catch(() => {}); // Swallow chunk load errors to not disrupt UX
        }
      }
    } catch (_err) {
  ...
  // Listen to intent signals: mouse hover and touch start
