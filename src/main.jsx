...
// Route chunk map for predictive prefetching
const routeChunks = {
  ...
  const prefetchRoute = (e) => {
    ...
    try {
      ...
      if (url.origin === window.location.origin) {
        // Use exact match or prefix matching (useful for parameterized routes if they existed)
        const path = url.pathname;
        const chunkKey = Object.keys(routeChunks).find(
          key => key === path || (key !== '/' && path.startsWith(key + '/'))
        );

        if (chunkKey && routeChunks[chunkKey]) {
          routeChunks[chunkKey]().catch(() => {});
        }
      }
    } catch (_err) {
