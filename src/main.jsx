...
// Centralized mapping of paths to dynamic import functions for lazy loading and predictive prefetching
const routeChunks = {
...
// Background prefetch logic: eagerly load the chunk when user hovers a link
  ...
  const prefetchChunk = (e) => {
    ...
    if (link && link.href) {
      try {
        ...
        // Only prefetch relative routes to the current origin
        if (url.origin === window.location.origin) {
          const path = url.pathname;
          // Exact match
          if (routeChunks[path]) {
            routeChunks[path]().catch(() => {}); // swallow chunk load error
          } else {
            // Find a matching directory prefix (e.g. /documents/123 -> /documents) if needed
            for (const key of Object.keys(routeChunks)) {
              if (key !== '/' && key !== '*' && path.startsWith(key + '/')) {
                routeChunks[key]().catch(() => {});
                break;
              }
            }
          }
