...
// Intelligent Route Prefetching (Phantom Infrastructure)
const routeChunks = {
...
if (typeof window !== 'undefined') {
  const prefetchRoute = (e) => {
    ...
    try {
      ...
      const match = Object.keys(routeChunks).find(
        key => path === key || (key !== '/' && path.startsWith(key + '/'))
      );

      if (match && routeChunks[match]) {
        // Trigger the chunk load in background and swallow errors
        routeChunks[match]().catch(() => {});
      }
    } catch (_err) {
      // Ignore invalid URLs
    }
