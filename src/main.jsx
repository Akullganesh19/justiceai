...
// Centralized mapping of routes to their dynamic imports for predictive prefetching
const routeChunks = {
...
// Predictive Route Chunk Prefetching
// Listen for user intention to navigate (hovering or touching links) and preemptively load the chunk.
  ...
  const prefetchChunk = (e) => {
    ...
    if (link && link.href) {
      try {
        ...
        // Only prefetch relative routing links on the same origin
        if (url.origin === window.location.origin) {
          const path = url.pathname;
          // Match path exactly or find a fallback if needed
          const importFn = routeChunks[path];
          if (importFn) {
            importFn().catch(() => {
              // Swallow prefetch errors so they don't pollute the console or break UX
            });
          }
        }
      } catch (_err) {
