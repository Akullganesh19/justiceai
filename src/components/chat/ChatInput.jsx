...
export default function ChatInput({ onSend, onUpload, isLoading }) {
  ...
  const { globalTranscription } = useTranscription();
  const lastProcessedTranscription = useRef(globalTranscription?.timestamp || 0);
  ...
  useEffect(() => {
    if (globalTranscription && globalTranscription.timestamp > lastProcessedTranscription.current) {
      setText((prev) => (prev ? `${prev} ${globalTranscription.text}` : globalTranscription.text));
      lastProcessedTranscription.current = globalTranscription.timestamp;
    }
  }, [globalTranscription]);
