...
export default function ChatInput({ onSend, onUpload, isLoading }) {
  const { transcriptionPayload } = useTranscription();
  ...
  useEffect(() => {
    if (transcriptionPayload?.text && transcriptionPayload.timestamp !== lastProcessedTimestamp.current) {
      setText((prev) => (prev ? `${prev} ${transcriptionPayload.text}` : transcriptionPayload.text));
      lastProcessedTimestamp.current = transcriptionPayload.timestamp;
    }
