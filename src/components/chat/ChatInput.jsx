...
export default function ChatInput({ onSend, onUpload, isLoading }) {
  ...
  const lastProcessedTranscription = useRef(transcriptionData?.timestamp || 0);
  ...
  useEffect(() => {
    if (transcriptionData?.text && transcriptionData.timestamp > lastProcessedTranscription.current) {
      setText((prev) => (prev ? `${prev} ${transcriptionData.text}` : transcriptionData.text));
      lastProcessedTranscription.current = transcriptionData.timestamp;
    }
  }, [transcriptionData]);
