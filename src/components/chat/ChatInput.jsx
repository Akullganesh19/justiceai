...
export default function ChatInput({ onSend, onUpload, isLoading }) {
  ...
  const lastProcessedTime = useRef(transcriptionData?.timestamp || 0);
  ...
  useEffect(() => {
    if (transcriptionData?.text && transcriptionData.timestamp > lastProcessedTime.current) {
      setText((prev) => (prev ? `${prev} ${transcriptionData.text}` : transcriptionData.text));
      lastProcessedTime.current = transcriptionData.timestamp;
    }
  }, [transcriptionData]);
